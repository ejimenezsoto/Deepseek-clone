...
const Sidebar = ({expand, setExpand}) => {
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen
      ...
      <div>
        <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
            ...
            className='group relative flex items-center justify-center hover:bg-gray500/20 transition-all duration-300 h-9 w-9 aspect-square rounded-lg cursor-pointer'>
            <Image src={assets.menu_icon} alt='' className='md:hidden'/>
            <Image src={expand ? assets.sidebar_close_icon : assets.sidebar_icon} alt='' className='hidden md:block w-7'/>
            <div className={`absolute w-max ${expand ? "left-1/2 -translate-x-1/2 top-12" : "-top12 left-0"} opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 roundedlg shadow-lg pointer-events-none`}>
        ...
        : " group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
            ...
            <div className='absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none'>
                New chat
                <div className='w-3 h4 absolute bg-black rotate-45 left-4 -bottom-1.5'>
                </div>
    ...
    <div>
        <div className={`flex items-center cursor-pointer group relative ${expand ? "gap-1 text-white/80 text-sm p2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer" 
